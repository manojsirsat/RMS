...
//				Thread.sleep(1000);
//				int totaloptions = webDB.getDriver().findElements(By.xpath("//ul[@id='batch-action-type-autocomplete_list']//li")).size();
//				log.logging("info", "Total options are "+totaloptions);
//				if(totaloptions>=0)
//				{
//					int oneoption = faker.number().numberBetween(1, totaloptions);
//					webDB.clickAnElement("//ul[@id='batch-action-type-autocomplete_list']//li"+"["+oneoption+"]", ElementType.Xpath);
//					Thread.sleep(1500);
//					
//					String selectedoption = webDB.getAttributeFromElement(BatchFilesPageLocators.BATCHACTIONTYPE_INPUTFIELD, ElementType.Xpath,"value");
...
//				log.logging("info", "Total options are "+totaloptions);
				if(totaloptions>=0)
				{
					...
					selectedoption = webDB.getAttributeFromElement(BatchFilesPageLocators.BATCHACTIONTYPE_INPUTFIELD, ElementType.Xpath,"value");
					log.logging("info", "Batch action type is selected as: "+selectedoption);
		    ...
		    case "Notification":
		        ...
		        if(flag)
		        {
		        	...
		        	if(flag)
		        	{
		        		...
		        		String errordetails = webDB.getTextFromElement(BatchFilesPageLocators.DETAILED_ERRORMESSAGE, ElementType.Xpath);
		        		if(flag)
		        		{
