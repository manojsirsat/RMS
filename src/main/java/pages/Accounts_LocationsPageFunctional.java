...
public class Accounts_LocationsPageFunctional {
	...
	public boolean fill_LocationForm() throws InterruptedException {
		...
		if (flag) {
			...
			flag = webDB.sendTextToAnElement(Accounts_LocationsPageLocators.CODE_INPUTFIELD, code, ElementType.Xpath);
			if (flag) {
				Thread.sleep(750);
	...
	public boolean click_CreateBtn() throws InterruptedException {
		...
		if (flag) {
			...
			Thread.sleep(1500);
			flag = webDB.waitForElement(Accounts_LocationsPageLocators.INVENTORYLINK_LOCATIONSUMMARYPAGE,
					ElementType.Xpath);
			if (flag) {
