...
public class Help_HelpFilesPageFunctional {
	...
	public boolean click_DownloadDocumentBtn() throws InterruptedException {
		...
		if (flag) {
			int totalcount = webDB.getDriver().findElements(By.xpath(Help_HelpFilesPageLocators.HELP_HELPFILES_DOWNLOADLINK)).size();
			...
			flag = webDB.clickAnElement(Help_HelpFilesPageLocators.HELP_HELPFILES_DOWNLOADLINK+"["+i+"]", ElementType.Xpath);
			Thread.sleep(5000);
